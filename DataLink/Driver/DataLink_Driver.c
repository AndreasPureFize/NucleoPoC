...
static bool dl_answer_device_reset(void)
{
  ...
  if (rx[0] != DL_TYPE_RESET || rx[1] != DL_OVERHEAD) return false;

  st = uart_read_exact(rx + 2, DL_CRC_SIZE, 50u);
  if (st != DL_OK) return false;
  if (crc16_compute(rx, DL_OVERHEAD, 0xFFFF) != 0) return false;
  ...
  while ((HAL_GetTick() - t) < 200u) {
    if (HAL_UART_Receive(&huart1, &tmp, 1, 20) != HAL_OK) break;
  }
  return true;
}
...
static bool dl_host_reset(void)
{
  ...
  if (st != DL_OK) return false;
  if (rx[0] != DL_TYPE_RESET_RESP || rx[1] != DL_OVERHEAD) return false;
...
{
  for (int i = 0; i < 20; ++i) {
    if (dl_answer_device_reset()) return true;
    HAL_Delay(25);
  }
  for (int i = 0; i < 20; ++i) {
    if (dl_host_reset()) return true;
    HAL_Delay(100);
  }
  return false;
}
    ...
    uint32_t headerWaitMs, uint32_t payloadWaitMs)
{
  ...
  if (st != DL_OK) return st;                             /* timeout/busy/link */
  if (rx[0] != DL_TYPE_READ_RESP) return DL_ERR_INVALID_RESPONSE;

  total = rx[1];
  if (total < (DL_OVERHEAD + 4u) || total > sizeof(rx)) return DL_ERR_INVALID_RESPONSE;

  st = uart_read_exact(rx + 2, (uint16_t)(total - 2), payloadWaitMs);
  if (st != DL_OK) return st;
  if (crc16_compute(rx, total, 0xFFFF) != 0) return DL_ERR_INVALID_RESPONSE;

  /* parse response payload: status(1), addr(2), size(1), data(size) */
  uint8_t  status = rx[2];
  uint16_t raddr  = (uint16_t)rx[3] | ((uint16_t)rx[4] << 8);
  uint8_t  size   = rx[5];
  if (status != 0 || raddr != addr || size != len) return DL_ERR_INVALID_RESPONSE;
...
dl_status_t dl_write(
    ...
    uint32_t headerWaitMs, uint32_t payloadWaitMs)
{
  if (len > 32u) return DL_ERR_INVALID_RESPONSE; /* same constraint as original */
  ...
  frame[4] = len;
  if (len) memcpy(&frame[5], inBuf, len);
  ...
  if (st != DL_OK) return st;
  if (rxh[0] != DL_TYPE_WRITE_RESP) return DL_ERR_INVALID_RESPONSE;

  uint16_t rtotal = rxh[1];
  if (rtotal != (DL_OVERHEAD + 4u)) return DL_ERR_INVALID_RESPONSE;

  /* tail is fixed 4 bytes here */
  uint8_t rtail[4];
  st = uart_read_exact(rtail, 4, payloadWaitMs);
  if (st != DL_OK) return st;

  uint8_t full[2 + 4];
  full[0] = rxh[0]; full[1] = rxh[1];
  memcpy(&full[2], rtail, sizeof(rtail));
  if (crc16_compute(full, rtotal, 0xFFFF) != 0) return DL_ERR_INVALID_RESPONSE;

  uint8_t  status = full[2];
  uint16_t raddr  = (uint16_t)full[3] | ((uint16_t)full[4] << 8);
  uint8_t  rsize  = full[5];
  if (status != 0 || raddr != addr || rsize != len) return DL_ERR_INVALID_RESPONSE;
